...
function DetailPanel({ state, data, links, callBridge }) {
  ...
  if (links && links.length > 0 && selectedEvent.id) {
    ...
    relatedEvents = relatedLinks.map(lk => lk.a.id === selectedEvent.id ? lk.b : lk.a);
    ...
    <div className={`detail-panel ${detailPanelCollapsed ? 'detail-panel--collapsed' : ''}`}>
      ...
      {!detailPanelCollapsed && (
        <div className="detail-panel__content">
           {/* Task 5.3.2: Add "UTC" suffix to timestamp labels for forensic clarity */}
           ...
           <Field label="App / File" val={selectedEvent.app_name || selectedEvent.fn_filename || selectedEvent.filename || selectedEvent.Source_Name || selectedEvent.name || selectedEvent.executable_name} />
           <Field label="Path" val={selectedEvent.app_path || selectedEvent.reconstructed_path || selectedEvent.Source_Path || selectedEvent.lower_case_long_path || selectedEvent.path || selectedEvent.target_path || selectedEvent.file_path} />
           ...
           <Field label="File Size" val={selectedEvent.file_size ? formatBytes(selectedEvent.file_size) : null} />
