...
# Define paths to the registry hives
...
last_update_regkey = reg_Claw(Software_reg_hive, last_update_path)
...
ComputerName_reg_key = reg_Claw(system_reg_hive, computerName_reg_path)
...
timezone_reg_key = reg_Claw(system_reg_hive, timeZone_path)
...
shutdown_reg_key = reg_Claw(system_reg_hive, shutdown_path)
...
searches_explorer_regkey = reg_Claw(Ntuser_reg_hive, searches_explorer_path)
...
recent_docs_key = reg_Claw(Ntuser_reg_hive, recent_docs_path)
...
typed_paths_key = reg_Claw(Ntuser_reg_hive, typed_paths_key_path)
...
lastsavemru_regkey = reg_Claw(Ntuser_reg_hive,last_savemru_path)
...
bam_key = reg_Claw(system_reg_hive, bam_path)
...
dam_key = reg_Claw(system_reg_hive, dam_path)
...
event_log_key = reg_Claw(system_reg_hive, event_log_path)
...
for name, (data, value_type) in event_log_key.items():
    cursor.execute('INSERT INTO EventLog (subkey, name, data, type) VALUES (?, ?, ?, ?)', ('vent_log', name, data, value_type))
