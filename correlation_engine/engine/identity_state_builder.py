...
class IdentifierCorrelationEngine:
    ...
    def process_evidence(self, extracted: ExtractedValues):
        ...
        # Build the full semantic dict BEFORE creating evidence rows so that
        # mutating it mid-loop doesn't propagate to anchors that already hold
        # an EvidenceRow reference. We also create a fresh EvidenceRow per
        # identity dispatch so anchors can't alias each other's semantic data.
        base_semantic: Dict[str, str] = {}

        if self.config.identifier_extraction.extract_from_names:
            for name in extracted.names:
                # 'name' field reflects the last name seen, matching prior behavior
                base_semantic['name'] = name

        path_identity_payloads: List[tuple] = [] # list of (identity_type, normalized_value, semantic_overrides)

        if self.config.identifier_extraction.extract_from_names:
            for name in extracted.names:
                identities = self.identity_extractor.extract_identities_from_name(name)
                for identity_type, normalized_value in identities:
                    path_identity_payloads.append((identity_type, normalized_value, {'name': name}))
            ...
            for path in extracted.paths:
                ...
                for identity_type, normalized_value in identities:
                    ...
                    if identity_type == "path":
                        ...
                        overrides['name'] = normalized_value
                        self.stats['names_extracted'] += 1
                    path_identity_payloads.append((identity_type, normalized_value, overrides))

        # Dispatch each identity with its own EvidenceRow + own semantic dict.
        for identity_type, normalized_value, overrides in path_identity_payloads:
